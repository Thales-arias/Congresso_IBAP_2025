...
# Ler o arquivo e deletar categoria outros devido ao baixo n
...
# Transformação de um banco só com os itens da eactd
...
# Proposição 01 de Wu e Estabrook (2016) 
# Somente fatores
...
# Proposição 04 de Wu e Estabrook (2016) 
# fatores e threshold 
...
# Proposição 11 de Wu e Estabrook (2016) - fatores, thresholds, cargas fatoriais e interceptos

prop11 <- measEq.syntax(configural.model = mod.eactd,
                        data = df,
                        ordered = colnames(eactd_ok),
                        parameterization = "delta",
                        ID.fac = "std.lv",
...
for (i in 2:4) {
  all.results[i, 7] <- all.results[i, 5] - 
  all.results[i-1, 5]  # Delta CFI
  
  all.results[i, 8] <- all.results[i, 4] -
  all.results[i-1, 4]  # Delta RMSEA
}

# Nomes das colunas e linhas

column.names <- c("chisq.scaled", "df.scaled", 
                  "pvalue.scaled", "rmsea.scaled", 
                  "cfi.scaled", "tli.scaled", 
                  "delta.cfi", "delta.rmsea")
row.names <- c("baseline", "prop4", "prop7", "prop11")
